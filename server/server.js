...
app.use(function (req, res, next) {
  ...
  AccessTokenModel.findForRequest(req, {}, function (err, token) {
    ...
    var UserModel = app.models.Device;
    UserModel.relations.accessTokens.modelTo
    .findById(token.id, function(err, accessToken) {
      if (err) return next(err);
      if ( ! accessToken){
      ...
      UserModel.findById(accessToken.userId, function (err, user) {
        if (err) return next(err);
        if (!user) {
          return next(new Error('could not find a valid user with the given token'));
        }
